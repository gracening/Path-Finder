...
public class LineSegment {
	...
	public final float k, m, b; // as in, ky = mx + b.
	...
	private boolean isPointInSegment(float x, float y) {
		return ((x >= Math.min(start.x, end.x) && x <= Math.max(start.x, end.x)) || FloatHelper.isZero(x - start.x)
				|| FloatHelper.isZero(x - end.x))
				&& ((y >= Math.min(start.y, end.y) && y <= Math.max(start.y, end.y)) || FloatHelper.isZero(y - start.y)
						|| FloatHelper.isZero(y - end.y));
	}
	 ...
	 * @return the point where the two segments intersect, or null if they do
	 *         not.
	 */
	public PointF findIntercept(LineSegment other) {
		...
		if (isPointInSegment(x, y) && other.isPointInSegment(x, y)) {
			...
			// x >= Math.min(other.start.x, other.end.x) && x <=
			// Math.max(other.start.x, other.end.x) &&
			// y >= Math.min(other.start.y, other.end.y) && y <=
			// Math.max(other.start.y, other.end.y) ){
	...
	public boolean containsPoint(PointF point) {
		if (!FloatHelper.areEqual(point.x * m + b, point.y))
			return false;
		// we know the point is on the line, if it is in the square denoted by
		// the start and end point, then it is in the segment
	...
	/**
	 * Calculates a unit vector that is parallel to this line segment, with the
	 * same direction
	 * 
	 * @return
	 */
	...
	/**
	 * This is semantically different from equality. It ignores direction. This
	 * is why I did not override .equals()
	 * 
	 * @param other
	 *            the other line segment.
	...
	public boolean theSame(LineSegment other) {
		...
		if (!(FloatHelper.areEqual(start, other.start) && FloatHelper.areEqual(end, other.end)
	...
	/**
	 * Weaker constraint than theSame(), returns true if the line segments
	 * overlap
	 * 
	 * @param other
	 * @return True if this and other are overlapping. False otherwise.
	 */
	...
	public LineSegment perpendicularLine(PointF point) {
		float X, Y = 0;
		X = point.x - 2 * this.m * (this.m * point.x - this.k * point.y + this.b) / (this.m * this.m + this.k * this.k);
		Y = point.y + 2 * this.k * (this.m * point.x - this.k * point.y + this.b) / (this.m * this.m + this.k * this.k);
		return new LineSegment(point, new PointF(X, Y));
	};

	public float distanceFrom(PointF point) {
		LineSegment perpendicularLine = this.perpendicularLine(point);
		PointF intercept = this.findIntercept(perpendicularLine);
		if (intercept != null) {
			return FloatHelper.distance(point, intercept);
		} else {
			return Math.min(FloatHelper.distance(point, this.start), FloatHelper.distance(point, this.end));
		}
	}
}
