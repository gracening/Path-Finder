...
public class FloatHelper {
	...
	/**
	 * Calculates the angle between two line segments. The two line segments
	 * that start at the same point
	 * 
	 * @param start
	 * @param end1
	 * @param end2
	 * @return the angle between the line segments [start, end1] and [start,
	 *         end2]
	 */
	...
	public static float getRotateAngle(PointF start, PointF end1, PointF end2) {

		final float epsilon = (float) 1.0e-6;
		final float nyPI = (float) Math.acos(-1.0);
		float dist, dot, degree, angle;
		float x1 = end1.x - start.x;
		float y1 = end1.y - start.y;
		float x2 = end2.x - start.x;
		float y2 = end2.y - start.y;
		...
		if (Math.abs(dot - 1.0) <= epsilon)
			...
			cross = x1 * y2 - x2 * y1;
			if (cross < 0) {
				angle = (float) (2 * nyPI - angle);
			}
		}
		degree = (float) (angle * 180.0 / nyPI);
		return degree;
	}
}
